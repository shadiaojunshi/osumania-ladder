...
## 2026-09-20：现在也会补「只缺 name」的图

判据从「beatmapId 不可用」放宽成「**beatmapId 不可用 _或_ name 是占位**」。
name 是占位 = 空、或整份就等于槽位记号（`ST1` / `RC1`）—— 早期"手传"时代的比赛
（ASC 2025 80/88、4DM2023 98/98…）BID 是对的但名字是槽位记号，上传页显示不出歌名，
旧判据永远够不到这批。

- **BID 绝不会被改写**：只有原来不可用（缺失 / 占位）时才写。
- **name 只补不覆盖**：已有真名的一律不动。
- 写回时的定位不再靠「BID 不可用」消歧，改成**要求该槽位在轮内唯一**，否则跳过。
- 影响量：旧判据 68 张 → 新判据 276 张（多 208 张）。

跑法不变，仍然**先 dry-run**：

```
Actions → Backfill Map Metadata → apply=false   # 只看报告
```
