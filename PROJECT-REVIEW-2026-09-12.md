...
- 已做的（对应上面第 1、2、4、5 条）：
  ...
  2. **生成结果带状态**：`generatePack` 返回 `{ realType, status, plannedSlots, packs:[{ key, status, reason, detail, skippedMaps }] }`；单包判定集中在 `evaluatePack`（`r2-upload` / `empty-pack` / `missing-entries` / `slot-count-mismatch`，其中 `empty-pack` 就是「只剩占位图」那种包）。内容不完整 → **不上传、不进清单**，并删掉本地临时包。
  3. **默认整次发布失败**：`summarizeRun` 汇总所有类型；只要有一项失败，就**不写 manifest、不写 `.previous`、不做任何清理**，打印失败清单并 `exit 1`（workflow 因此到不了 commit 那一步）。
  4. **链接改为合并**：`mergeLinks` 以旧链接为底、本次新链接覆盖同名键。过去写的是 `result.links || previous.links` —— 本次只产出 r2，所以是**整体替换**，会把每个包的 `googleDrive` 链接抹掉。另外：内容确实更新过的包记进 manifest 顶层 `pendingMirrors`，由 `upload-to-gdrive.js` 在成功上传后逐个划掉（失败 / 未涉及的保留标记）—— 既不丢人工镜像链接，也不会让旧镜像冒充新版本。
  5. **清理默认只报告**：`findOrphanKeys` 只负责算；真删必须显式 `--clean-orphans`，且位置在写 manifest 之后。理由就是第 5 条的原话：删早了、而 manifest 的 git push 失败，线上清单还在引用这些键，下载页立刻断链。
- 测试：新增 `pack-publish.test.mjs` **18 例** —— 三个验收场景（第 N 张谱面损坏 / 第 N 包上传失败 / 只有占位图的包不发布）、`skipped` 与 `failed` 必须分开、links 合并、`pendingMirrors`、分包数缩小时消失的 part 不残留；另加两类「守门」断言：**真实 `data/packs-manifest.json` 的不变量**（全量成功时 55 个条目的链接与 Drive fileId 一个都不丢）与**动作顺序**（判定失败必须早于写 manifest、删对象必须在开关之后、旧的 `allResults` 路径不得残留）。变异测试（把门控改成无条件发布 + links 改回整体替换）→ **18 例里 7 例失败**，恢复后 18/18，证明断言有区分度。
- 运行的验证：`npm test` → **359/359**；前后端 `tsc` 0 错；`npm run build` 成功。
- 未完成 / 仍有风险：
  ① **第 3 条（版本键隔离）未做** —— R2 对象键仍是 `realType_part.osz` 的覆盖式写入。generate 阶段的上传发生在 manifest 提交**之前**，所以若上传成功而随后的 commit 失败，桶里内容已换、线上清单还是旧的（mapCount 对不上）。彻底解决要 runId/内容哈希键 + 「全部校验成功后再发布」，与 R12 的串行发布一起做。
  ② 单类型模式（`--type=`）仍是「更新对象但不更新清单」（R12 第 3 条）。
  ③ **行为变化须知**：全量跑不再自动删孤儿包。`generate-packs.yml` 这轮没改，所以 workflow 现在也不会删 —— 需要清理时要么本地跑一次带 `--clean-orphans`，要么之后给 workflow 显式加参数。
  ④ 未在真实 R2 上跑过全量；判定逻辑靠纯函数单测 + 源码结构断言覆盖，不是端到端证据。
...
- 实现要点：
  ...
  2. **manifest 条目新增 `objectKey`**：本次产出的用新键；未涉及 / 未重新生成的条目（含 `skipped` 类型与 `preserveOtherTypes` 保留的那些）**沿用旧键**，不会被抹掉。
  3. **孤儿判定改为「清单实际引用的键」**：没有 `objectKey` 的历史条目按其旧命名算被引用 —— 否则升级后第一次清理会把它们全删掉。这同时意味着：上新键而清单**未提交**时，新键会被列成孤儿、旧键仍被引用（默认只报告，不会误删）。
  4. **Drive 侧**：从 R2 拉流用 `entry.objectKey`（回退旧命名）；Drive 上的**文件名仍是 `realType_part.osz`** —— 与内容寻址无关，避免重传时在 Drive 里新建副本。
  5. 离线预览会打印「假如发布会用哪个键」；孤儿报告里明确提示上一版旧键也会出现在列表里。
- **过渡须知（给站长）**：改用新键后，**上一版的全部旧键对象都会变成孤儿**。默认只报告不删 —— 确认新 manifest 已提交、下载页正常之后，再跑一次 `--clean-orphans` 回收即可。
- 测试：`pack-publish.test.mjs` **36 例**（新增 6 例：键格式与内容复用、孤儿按引用判定、历史条目不误判、未提交清单仍指向旧键、`objectKey` 的传递与沿用）。**变异测试**（键忽略哈希 / 孤儿退回旧拼法 / manifest 不带 objectKey）分别挂 1、2、2 例，恢复后 36/36。
- 至此 R10 的 6 条全部落地（第 3、6 条即本项）。运行验证：`npm test` → **407/407**；前后端 `tsc` 0 错；`npm run build` 成功。

**复查追加修正 4（2026-09-18）：另一条线的安全审查指出 5 处问题，已全部加固**

背景：另一条工作线在 `docs/security-review-and-recovery-2026-09-18.md` 里单列了一节「合包交接给另一 AI」，对上一轮的发布链提了 7 条意见。逐条实测后，5 条成立，已修：

1. **`--type=` 空值、裸 `--offline` 会静默走成全量发布**（P1）。实测 `parsePackCli(['--offline'])` 返回 `mode=full` —— 用户以为在预览，实际会全量上传；`--type=` 与 `--type=  `（空白）同理。改成硬错误：`empty-type` / `offline-requires-type`，另加「重复指定不同类型」检查（`duplicate-type`）。验收：三种输入全部 `exit 2`、零写入。
2. **孤儿清理挂在生成流程末尾、且按「本次生成」的清单判孤儿**（P1）。真删**移出生成流程**，拆成独立命令 `scripts/gc-pack-objects.mjs`：读**已提交/已部署**的清单、带**保留期**（默认 24h，时间未知的对象更不动）、默认只报告。`generate-pack.js` 里的 `--clean-orphans` 现在直接报错并指向新命令。理由：重跑一次若 ZIP hash 变了，新键与线上被引用的键不同 —— 按本次清单判孤儿会删掉**线上正在引用**的对象。
3. **Drive 的孤儿删除发生在 manifest 提交之前**（P1）。脚本无法知道 workflow 的 `git push` 是否成功，线上旧清单可能仍引用那些 fileId。改成默认只报告，真删要显式 `--clean-orphans`。
4. **`pendingMirrors` 每轮从空 Set 重建**（P2）。改成**只增不减**：带上旧标记（那些包的镜像此刻仍指向旧内容），只丢弃「对应条目已不在清单里」的标记；由 Drive 同步成功后清账。
5. **packs 桶的 LIST 没分页**。补 `listPackBucketObjects()`（对象数可能超过单页 1000）。

另按它的建议：workflow 在 `failure() || cancelled()` 时 `upload-artifact`，保存 manifest / previous / 身份报告（不含凭据）—— 否则 runner 一退，那句「已保留本地结果」就是空话。

**仍未做**：Drive 侧镜像仍是「按文件名原地 update」，所以**镜像不是原子的**（第 N 包失败或 push 失败时，旧 Drive 链接的内容已经变了）。已用只增不减的 `pendingMirrors` 标出，但**下载页还没消费这个字段**。它给的两个方向（Drive 也版本化 / 在消费端展示「镜像未同步」）待定。

测试：`pack-publish.test.mjs` 扩到 **42 例**（新增：裸 `--offline`、空与空白 `--type=`、重复类型、`pendingMirrors` 只增不减与下线丢弃、孤儿保留期与「时间未知不删」；结构断言改为「生成流程不含 `DeleteObjectCommand`，且报告指向 GC 命令」）。变异测试 5 处（裸 offline / 空 type 的两处检查 / pendingMirrors 重建 / 忽略保留期）均被对应断言抓到。CLI 与 GC 脚本退出码实测：非法参数 2 / 缺凭据 1 / `--help` 0。`npm test` → **413/413**；前后端 `tsc` 0 错。

**复查追加修正 5（2026-09-18）：Drive 镜像也改成版本化 —— 两个渠道彻底对称**

- 背景：修正 4 里列为「未做」的那条（Drive 按文件名原地 update，镜像非原子）。站长确认 Drive 有 5TB 空间 —— 原先否掉版本化的唯一理由是副本堆积，这个理由不成立，遂实施。
- 实现（`scripts/upload-to-gdrive.js`）：
  1. **Drive 文件名改用内容键**（与 R2 同一个 `{realType}_{part}.{hash8}.osz`）→ **同名即同内容**。
  2. 上传前三级复用：① 清单里 `gdriveObjectKey` 与本次 `objectKey` 相同 → 直接复用（**跳过上传**）；② Drive 上已有同名文件（上次跑到一半、或清单被回滚过）→ 复用；③ 都没有才新建 + 设 anyone-reader。**重跑能省下整轮上传流量** —— 这是版本化顺带来的收益。
  3. **历史条目（没有 objectKey）仍按名字覆盖**：否则 Drive 里会多出同名副本，而线上旧清单的链接还指着老 id。
  4. 旧版本文件只进孤儿报告（默认不删，沿用已有逻辑）；判定按清单实际引用的 fileId，版本化后自动正确。
  5. manifest 条目新增 `gdriveObjectKey`（`buildManifestPacks` 继承），Drive 侧靠它判断「是不是已经是这一版」。
- 效果：两个镜像都变成「新内容存新文件、清单最后一次性切」。中途失败最多是**留着旧版本**，随时可回滚；不再出现「清单是新的、Drive 是旧的」。
...
- 已做的（对应上面 1–6 条）：
  ...
  2. **同一个 BID 也要核对内容**：`pathsNeedingContentCheck` 挑出「同一 BID 出现在多个物理路径」的引用去读内容；**先用 R2 对象大小预筛** —— 同一 BID 的两个副本大小不同则内容必然不同（.osz 是确定的字节序列），直接判为「不合并 + 报冲突」，不必各下载一次。单一路径的 BID 不多花一次下载。
  3. **备选路径按内容等价兜底**：`tryPathsInOrder` 按 `alternatePaths` 依次尝试（下载失败 / 解压失败 / 没有 .osu / 内容摘要不符 都算失败），且**摘要与簇的基准不一致的备选会被拒绝**（绝不拿别的图冒充）。过去只按 `r2Keys.has()` 挑一个路径，首选文件损坏就直接丢图。
  4. **来源标签不再丢**：主图即使 R2 里没有也收进 `rawEntries`（标 `exists:false`）；聚簇后**只有身份能唯一确认**（同 BID 且该 BID 下只有一个簇）才把它的来源挂到那个副本上，否则进 `unresolved` 清单 —— 不再像过去那样在 `rawEntries` 之前 `continue` 把标签丢掉。
  5. **计数同口径**：`slotTotalOf` 只数主图簇（NSV 变体不算新槽位），与 manifest 的 `mapCount`（包内非 NSV 条目数）一致。过去按 `bid:` / `raw:r2Key` 去重，合并后的簇会被算成多份，分母永远追不上分子。
  6. **人工核对项落盘**：`reports/pack-identity-report.md`（同 BID / 同元数据但内容不同、缺文件且身份无法确认），运行日志同步打印。本项**不自动改 JSON 的 BID/键型，也不动线上包**。
- 影响面（本地 52 场 / 4750 槽位自查）：**95.2% 的槽位有可用 beatmapId**（身份由 BID 决定，根本不走元数据指纹）；**4.8%（229 个槽位）没有可用 BID** —— 这正是过去只能靠元数据指纹合并的风险群体；另有 **287 组**「同一 BID 被多个槽位复用」（现在按大小预筛、必要时读内容核对）。用 `.workbuddy/tmp/osu` 的 204 张真实 .osu 抽查：0 组「同元数据不同内容」（这些样本本来就互相独立），但发现 **3 张元数据全空的空谱** —— 旧逻辑下它们会得到同一个 `fp:|||` 而被并成一张，新逻辑各自独立。
- 测试：`map-identity.test.mjs` **21 例** —— 内容摘要的忽略/保留边界（元数据、背景、音频名不同仍相等；音符、时间轴、OD、Mode 不同则不等；只有注释与空白差异仍相等；空谱摘要为 null）、空元数据不合并、同 BID 同内容合并 / 不同内容报冲突、NSV 不与主图合并、缺文件挂靠与 `ambiguous-bid`、大小预筛、计数口径、选路、备选命中 / 全套失败 / 内容不符被拒。**变异测试**：① 内容摘要不参与合并键（退回「只看元数据」）→ 4 例失败；② 计数把 NSV 也算上 → 1 例失败；③ 选路不判存在性 → 1 例失败；各自恢复后 21/21。
- 运行的验证：`npm test` → **380/380**；前后端 `tsc` 0 错；`npm run build` 成功。
- 未完成 / 仍有风险：
  ① **未在真实 R2 上跑过全量** —— 内容摘要的粒度、大小预筛的命中率、报告内容都只在单测与 204 张缓存样本上验证过。
  ② 首次全量会为「同 BID 多路径且大小相同」的图多做一次下载核对（大小不同的已被预筛跳过），时间与流量会比过去略增。
  ③ `contentSignature` 的「忽略哪些段」是写死的规则（`CONTENT_SECTIONS` / `GENERAL_KEEP`），将来若判定要忽略别的字段得改那两处。
  ④ 单类型模式（`--type=`）的身份报告会覆盖全量报告（只含该类型）。
...
- 已做的（对应上面 1–5 条；第 6 条见下）：
  ...
  2. **CLI 校验**：参数解析移到 `pack-publish.js` 的 `parsePackCli`（纯函数、可单测）。大小写不敏感匹配但**返回规范键**（`ss`→`SS`、`rcmainhb`→`RCmainHB`、`mxhb`→`MXHB`，不能 toUpperCase）。**未知选项与未知类型都 exit 2 并列出可用类型**（过去静默忽略，打错成 `--type-ss` 会悄悄变成全量发布）；Pending 族（`PDRC` 等）是「合法但不产包」→ 警告而非错误。凭据检查也从模块顶层挪进 main —— 否则 `--help` 和参数报错都会被一句「缺少凭据」挡住。
  3. **单类型语义明确化**：`--type=X` 默认**仅离线预览**（生成到 `output/`，不上传 R2、不动 manifest），从根上消除「更新了对象却没更新清单」；`--type=X --publish` 才是发布 —— 上传 R2 + **只替换该类型的 manifest 条目**（新增 `preserveOtherTypes`，其他类型与其人工链接、`gdriveFileId` 原样保留）+ 标 `pendingMirrors`，且**不写 `.previous`、不清理孤儿**（这两件只有全量做，否则 Drive 侧会照一份不完整清单去算孤儿）。单类型模式下 `--clean-orphans` 被强制忽略并警告。
  4. **并发协调**：两个 workflow 共用 `concurrency: group: packs-publish` + `cancel-in-progress: false`（排队而不取消 —— 取消会留下「包已传、清单没更新」的中间态）；Drive 侧还会读取 `RUN_ID` 作记录。
  5. **commit/push 条件化**：提交条件拆清楚（全量，或单类型 `--publish`；纯预览不产生提交）。manifest 无变化直接跳过；push 失败先 `fetch` + `rebase` 再试、**最多 3 次**；rebase 冲突就 `--abort` 并**保留本地提交与生成结果**、以 `::error::` 报错退出 —— **全程没有 force push、没有回滚，也不会用旧 manifest 覆盖人工链接**。
- 第 6 条（版本对象键）**未做**：它依赖 R10 第 3 条（runId / 内容哈希隔离的对象键），与「全部校验成功后再发布 manifest」是同一件事，留到下一轮统一处理。本轮没有硬编码旧命名 —— 对象键 `realType_part.osz` 只出现在 `pack-publish.js`（`findOrphanKeys` / `buildManifestPacks`）与 Drive 的文件名，改键时只需动这几处。
- 测试：`pack-publish.test.mjs` 扩到 **30 例**（新增 12 例：三种 mode、大小写不敏感与混合大小写规范键、含 `;` / `&&` / `$( )` / 反引号 / 换行 / 空格的输入一律被拒且不成为 targetType、未知选项报错、`--publish` 缺 `--type` 报错、`--publish` 与 `--offline` 互斥、Pending 族只警告、单类型忽略 `--clean-orphans`、`--help`、`preserveOtherTypes` 保留其他类型及其 `gdriveFileId`/镜像链接、单类型发布标 `pendingMirrors`）。**变异测试**（白名单失效 / 未知选项静默忽略 / 不保留其他类型 / 单类型也清理）→ 每个变异各挂 1 例，恢复后 30/30。
- 运行的验证：`npm test` → **392/392**；前后端 `tsc` 0 错；`npm run build` 成功；两个 workflow 用 js-yaml 解析通过（`concurrency` 与 inputs 均正确）；CLI 退出码实测（非法参数 2 / 缺凭据 1 / `--help` 0）。
- 未完成 / 仍有风险：
  ① **未在真实 Actions 上跑过** —— 排队行为、rebase 重试、`::error::` 注释只做了静态与本地模拟验证。
  ② 第 6 条版本键未做（见上）。
  ③ 单类型发布后 Drive 镜像要等下一次整套重传；清单里的 `pendingMirrors` 目前只给人工核查与 Drive 脚本用，下载页不消费这个字段。
...
### R34 [P2] 键型谱面页反复报"同一张图被多个轮次复用" —— 曲名栏写的其实是**槽位记号**（站长反馈）

来源：站长 2026-09-19 —— 后台「键型谱面」页一直顶着 **SSR SF/F** 的名字报重复，而 SSR 的两个轮次（`round-8` / `round-8-f`）曲名与 BID 全不同，本就不该报警。

两层原因，第二层才是真的：

1. **文案层**：横幅把 SSR 写死成例子，而显示条件是「当前筛选里任意一条重复就显示」→ 别的比赛触发时顶着 SSR 的名字。
2. **判读层（根因）**：`mapIdentityKey` 在 `name` 非空时用「大类 + 曲名 + 难度」当身份，但全库有 **227 张**图的 `name` 根本不是曲名而是**槽位记号**（`SV1` / `ln3` / `RC8` / `TB1`，集中在 4DM2023 / ASC 2025 / TTI / MCNC 2025 …）。「同大类 + 同槽位名 + 同难度」在不同的**独立轮次**之间必然相撞 —— 4DM2023 的 SV1 就是这么在 **7 个轮次**里被报成"同一张图被复用了"。

实测影响面（改动前后都用全库真实数据跑过，不是估的）：

| | 条数 | 构成 |
| --- | --- | --- |
| 改前 | **9** | 6 条占位名误报（4DM2023 SV1 跨 7 轮 / SV2 跨 6 轮 / RC2 跨 2 轮；ASC 2025 ST1-SV1 跨 6 轮 / LN3 跨 2 轮 / LN4 跨 2 轮）+ 3 条 COEMT 真复用 |
| 改后 | **3** | 全部是 COEMT（`Feryquitous - Ai Nov [Crumbling]` / `Schwank - HULLABALOO WRECKING CREW` / `Yuyoyuppe feat. Hanatan - Palette [x1.2]`）在 `RO16&QF` 与 `SF&F` 两轮之间**确实放了同一张图** —— 保留 |

**完成记录（2026-09-19）**

- 状态：实现完成待验收（已随本批提交入库）。改的是**判读**，不改包、不改比赛数据 —— 数据侧 0 处改动。
- 修改文件：`src/lib/tournamentDiagnostics.ts`、`src/components/admin/RealTypeMapBrowser.tsx`、`src/lib/messages.zh.ts` / `messages.en.ts`、`scripts/admin-data-quality.test.mjs`（+5 例）。
- 实现要点：
  1. **占位名判读** `isSlotPlaceholderName(name, slot)`，两条判据命中任意一条即视为占位：① `^[a-z]{1,4}\d{1,3}$` 的短记号（**要求至少一位数字**，避免误伤 `MU` 这类短曲名）；② 归一化后与**自己的槽位名**完全相同（兜住 `FS/TB`、`GM(HR/SD)` 这种带符号的槽位）。命中后忽略 `name`、身份退到 BID；**没有可用 BID 就当"没有身份"**（`mapIdentityKey` 返回 `null`）—— 宁可漏报，也不拿槽位名当曲名去误报。
  2. **轮次判重改用 `round.id`**（原来按 `abbreviation`）：两轮缩写都叫 "F" 时，按显示名去重会把两轮合成一轮、把**真正的复用吞掉**（漏报）。`rounds` 数组仍是**给人看的**，所以按显示名去重（否则会输出 `F & F`）→ **警告条目存在即等于"确实跨了 ≥2 个轮次"**，调用方不得再拿 `rounds.length` 当门槛。
  3. `RealTypeMapBrowser.tsx` 的横幅与行内 `!` 判据跟着从 `rounds.length > 1` 改成"条目存在"，与上面那条契约对齐（否则缩写重名时行内提示会消失）。
  4. 文案（zh/en）补上「曲名只是槽位记号（如 sv1 / ln3）时不当作身份」/「Slot placeholders such as sv1 / ln3 are not treated as identity」。
- 运行的验证（本次实测，不照抄笔记）：全库 **55 文件 / 385 轮 / 4987 张谱面**跑新旧两版口径对拍 → **9 → 3**（逐条列在上表）；`npm test` → **519/519**；前后端 `tsc` 各 0 错；`npm run build` 成功（`/`、`/admin`、`/download` 三条路由静态预渲染）；`npm run lint` **25 条存量错误**、与 R19 基线一致（未新增）；密钥扫描 0 命中；**浏览器实机验收**（系统 Edge + `out/`）：天梯视图 14/14、后台键型谱面 10/10（横幅点名 COEMT 而不是写死的 SSR、不再出现 4DM2023 误报）。
- 未完成 / 仍有风险：① **已知边界，刻意不动**：`name` 非空且是**真实曲名**时，身份只看「名字 + 难度」、不看 BID —— 所以"同 BID 但两轮曲名写法不同"**不会报**（R33 定下的"名字优先"契约，测试里有用例锁住并注明是刻意保留）；② 占位判读的两条判据是**启发式**：新出现的槽位命名风格（纯符号、5 个以上字母等）可能漏判，届时补判据；③ 全库剩下的 3 条是**真复用**，属数据本身的问题（同一张图放在两个轮次），要不要动由站长定，本轮只报告。
## 6. 可靠性与局部修复任务
...
**第六批（天梯身份层 + 悬浮锁定位 + 三处纯逻辑抽离）已提交入库**：
- **整场比赛视图成为默认**（`viewStore.mode` 由 `round` 改成 `tournament`）：框内显示比赛**全名**而不是 2–6 字母缩写，一个按钮承载整段难度范围。
- **悬浮锁定位**（`src/lib/ladderHover.ts` 的 `updateLadderHover`）：整场视图下，同一次悬浮会话**固定卡片坐标**，鼠标移动/滚动只换命中的轮次；换到另一场比赛才重新定位。每轮 / 键型视图保持旧行为（`lockPosition=false` 时卡片跟随指针）。滚动重绘走 `requestAnimationFrame`（滚动不发 `mousemove`，过去卡片会一直描述已经滚走的那一轮）。指针落在卡片上时 `elementFromPoint` 先命中 `[data-ladder-hover-card]` 就直接 return，不误判成"离开了框"。
- **`RangeSurface` 单表面**：比赛框与每个轮次框共用 `computeRangeSurface` + `rangeSurface()`（`src/lib/ladderGeometry.ts`、`src/components/ladder/RangeSurface.tsx`）—— 统一渐变坐标（`backgroundSize = 绘图区高`、`backgroundPosition` 由表面顶偏移），熔岩头包含在按钮内部，不再有"残片副框"。
- **比赛图标**（`src/lib/tournamentEmblem.ts` + `TournamentIdentity.tsx`）：`scripts/generate-tournaments.js` 构建时扫 `public/tournament-icons/<完整 id>.<ext>` 生成 `src/generated/tournamentIcons.ts`（图标本身**要入库** —— .gitignore 只忽略生成物 `src/generated/`）。**无图标就不挂图片、不发请求、不启观察器/计时器**，不做 404 探测。首次可见且加载成功后保留原貌 5 秒再淡入徽章；`prefers-reduced-motion` 下同样等 5 秒但不做过渡。
- **三处纯逻辑抽离**（公开反馈页要复用，见 `docs/anonymous-feedback-and-abuse-plan.md` 第 8 节）：`src/lib/roundReference.ts`（整轮参考的唯一实现，后台编辑器 + 未来公开页共用）、`src/lib/mapBrowserRows.ts`（键型浏览表格的行生成/筛选/排序/下拉项）、`src/lib/suggestions/{types,validation}.ts`（建议提交的判别联合 + 白名单校验，与 `functions/api/_lib/{mapKeys,tournamentId,validation}.ts` 是**两份独立实现**，靠 20 例镜像测试锁死"逐例同判"）。`realTypeCatalog.ts` 新增 `STANDARD_MAP_CATEGORIES` / `categoryOfRaw` 作为大类判读的**唯一实现**。
- **本轮审查线的独立验证**（不是照抄文档）：全库 385 轮中**80 轮**没有任何难度值（逐图全 0 且 `round.difficulty` 三项全 0），在**新旧两版都不渲染**（同样提前 `return`）→ 新实现"不可见即不渲染"**零新增隐藏**；标签位置仅 3 处移动且 ≤10px（夹取口径从 `rawTop` 改成 surface 顶所致）。浏览器实机 **15/15**（默认整场视图、全名身份层、单表面背景坐标、悬浮卡片坐标固定 496,492→496,492、卡上可点、视口夹取、离开消失、切轮次视图标题层回来且 305 条标题全落在框内、每轮框仍各自着色、换框后卡片重新定位 496,492→792,672、键型视图 1153 框）。
**第五批（分包方案 + 搜索补全）已提交入库**：
- **合包切包方案**（站长「不喜欢第一个包里全是 MWC」）：新增纯函数 `splitIntoPacks` / `buildAtoms` / `groupByTournament` / `resolveSplitMode`。排序、包数、每包张数**都不变**，只换成员。三档：`sequence`（旧行为，连续切，留作回退与对照）、`tournament`（**默认**，整场比赛为一张牌发给剩余容量最大的包；超过一个包 1/8 的「大场」先摊成 P 段；一张牌装不下就切开、余量给下一个最空的包）、`entry`（逐张轮转的混沌版）。共同不变量：容量守恒且不符直接抛（绝不静默丢图）、NSV 与其主图同包、**确定性**（同一份数据每次得到同样划分 —— 绝不用随机数，`Pack N` 写进每张图的 `Title`，随机种子等于每次发布都动玩家成绩身份）。
  ⚠️ **换方案 = 换包成员 = 换包号 = 换每张图的 `Title`/`Version` = 已下载旧包的玩家断成绩**。线上 55 个包（清单 `lastGenerated` 2026-08-13）用的是旧的固定 80 切块 + 旧顺序，所以**第一次用新链发布本来就会大面积变**（离线评估：48~78% 的图换包号）。与 `compareTournamentsForSources` 是同一个坑，要换就一次换定。留了个待定项给站长：要不要顺手去掉 `Title` 里的 ` Pack N`（一次性断成绩，换以后随便重排包都不动 `.osu`）。
- **搜索补全**（站长「搜比赛全名也搜不到 / 搜一个谱面的任何信息都能被搜到」）：真因**不是匹配不上，而是「整场命中一行都不产出」** —— 所有词都落在比赛名/缩写里时 `rounds`/`maps` 被刻意清空（防刷屏），面板显示「找到 1 场比赛 · 0 个轮次命中 · 0 个谱面命中」下面一行可点都没有。55 场里 **48 场**如此。改法：新增 `tournamentMatch` 字段 + 面板补一行「整场比赛」入口（原地展开该场轮次）；`searchKey()` 把**标点折成空格**（`osu!mania` / `osu mania` / 全角逗号 tag / 连写 `osumania` 都能搜到，连写那半靠内部 id 接住）；索引扩到整场的 `id`/`year`/`keyCount`/`tags`、轮次的 `id`、谱面的槽位/大类/键型（后者是**代号**，按词首匹配 —— 任意子串会把 FCJ 的图当成 `CJ` 的命中）。
  `difficulty` **没进索引**：小数点在折标点时变成空格（`6.4` → `6 4`），要单开一条数字通道，收益小、契约变复杂，先不做。
- **本轮我（审查线）另修的一个 bug**：`searchKey` 折完标点为空的查询（输入 `!` / `&`）原先走了「空白查询」分支，把**全库 55 场**当成无行结果返回 —— 面板显示「找到 55 场比赛 · 0 个轮次命中 · 0 个谱面命中」下面全空，天梯列还从「`!` 筛出 52 场」变成「显示全库 55 场」，看起来就像搜索框没反应。根因是「有内容但被折没了」与「空白查询」被混为一谈：空白查询是**天梯列的契约**（要原样返回全表），纯标点则是「搜了但无可匹配的词」→ 现在返回空结果。新增 2 例测试锁住这条判据（含「空白查询仍原样返回全表」那一半）。
**第四批（跨比赛元数据暂存 + 轮次检索）已提交入库**：见下方「未验收边界」两条。

**仍未验收（2026-09-19）**：
- **跨比赛元数据暂存**（站长反馈「必须一个比赛保存一次」）：补丁池从 `StagedPatchMap` 改成 `StagedGroups = Map<tournamentId, StagedPatchMap>`，统一保存走已有的 `POST /api/tournaments/batch`（每场各自带 `baseSha` 编辑基准，一个请求 = 一次 commit = 一次重建）；撞 409 整批重读最新 sha 自动重投一次；池子落盘 `localStorage`，因此切 tab / 刷新都不丢。单场那层的纯逻辑（`fill` 不覆盖远端、快照清池）一字未改。
- **轮次级检索**（站长反馈「搜不了 MMT SF 这一整个轮次」）：`TournamentSearchResult` 新增 `rounds` 字段，轮次的 `name` / `abbreviation` 首次进索引；点轮次行直接打开该轮图池。唯一刻意的不对称：轮次文本用**词首匹配**（搜 `F` 不会把 `Semifinals` 拖出来），比赛名 / 谱面名仍是包含匹配。

两条的共同边界：**只有纯函数单测与浏览器实机验收，没有线上联调** —— 没有真实 GitHub 凭据，`POST /api/tournaments/batch` 的真实写入路径（含 409 分支）只在桩上验过。

进度速览（2026-09-19）：**本文件此前各处"未提交"的标记全部过期 —— 实现均已提交并推送**（`2ca51d9` 占位 ID 判读 + 管理员角色 + 上游错误分类 + 删除前备份 + 请求体实流计数；`67ebdcf` 合包发布链；`cd8d96e` MKTC 36 条占位 setId 清理；`419c4b4` 检测报告刷新）。以下只保留每项**尚未验收**的边界：✅ R01（UI 自动重放未做）、✅ R02（浏览器端未联调）、✅ R03（已提交 `3634998`，未线上联调）、✅ R04（未线上联调）、✅ R05（时序行为无自动化测试）、✅ R06（**「替换当前版本」入口未做**、未线上联调）、✅ R07（并发删/重传未验收）、✅ R08、✅ R09、✅ R18、✅ R24（下载路径网络重试）、✅ R25（悬浮卡段位与框高同源）、✅ R26（不参与难度统计 + 整场视图按指针选轮次 + 版本 0.9.0）、✅ R27（键型冲突改“勾选才改”+批量选择）、✅ R28（PDEX 键型 + 默认键型一律 Pending + 误标检测工具，检测结果需人工复核）、✅ R29（悬浮卡 TB 段位改用实际难度，未做浏览器验收）、✅ R30（TB 段位规则细化 + 每轮图池标题折行；MCNC 简写查为残留记录，数据未改）、✅ R31（合包分包规则均分 + `$` 注入修复 + NSV 缺音频借用主图；未重新生成包）、✅ R13（状态 API 分页 + 失败不返回部分清单）、✅ R16（审计 metadata 按字节收缩 + 失败可见 + 可按 key 看全文）、✅ R17（LN 参考读数改用 difficulty，两处实现合并）、✅ R14（上游故障不再伪装成空数据/404/401，404 探仓库可见性，前端不再崩且失败保留旧数据）、⚠️ R15（原型链 role 拒绝 + 脏数据净化 + 缓存返回副本 + 写失败不动缓存 + 一致性边界改成 60s+5s；**并发丢更新未修**，需强一致存储）、✅ R14 复查追加（上传/批量保存 + 四处写路径全部接上分类：凭据失效不再报「比赛不存在」也不再回 401，批量上游故障回 502 而不是 500；过程修掉一个 `res.clone` 已被消费导致 500 的真 bug）、✅ R32（删除 .osz 后该 slot 的本地 name/BID 与暂存补丁一起回滚 + 删除后对账已上传状态）、✅ R33（占位 ID 判读 + 冲突检查器/下载路径防护 + 检测与清理脚本；**MKTC 那 36 条数据已清理并推送** `cd8d96e`）、✅ R10（合包发布门控 + **内容寻址的对象键** + 审查后加固：GC 拆成独立命令带保留期、裸 `--offline`/空 `--type=` 直接报错、**Drive 也改内容寻址并跳过已上传**：任一包失败则整次不发布、占位包不上传、镜像链接合并 + `pendingMirrors`、孤儿清理默认只报告；未在真实 R2 上跑过全量）、⚠️ R11（谱面身份：元数据降级为候选键、等价性改由内容摘要决定，同 BID 也核对内容并输出人工核对项，备选路径按内容等价兜底、来源标签不丢、计数与 mapCount 同口径；未在真实 R2 上跑过全量）、✅ R12（workflow 输入改 env + 数组传参、CLI 白名单校验、单类型区分「离线预览 / 发布」两态、两个发布 workflow 共用 concurrency 组、push 冲突保留本地结果；第 6 条已随版本键落地，未在真实 Actions 上跑过）。

**仍待实施（2026-09-19）**：R23 的**发布验收**（只读核实已完成，见其完成记录）；R15 的**并发丢更新**仍在等强一致存储（Durable Object）；合包链的**端到端故障注入**与**真实 R2 全量实跑**（见 `HANDOFF-PACKS-AND-REMAINING-2026-09-18.md` §3.A）。

**第二批（R19 + R20 + R21 + R22）已提交入库**：R19 = `eslint.config.mjs` + `scripts/serve-static.mjs` + 测试 + `package.json` scripts；R22 = `.gitignore` 加 `tsconfig.tsbuildinfo`（已 `git rm --cached`）、`roundLabelLayout.ts` 死算法与 `scripts/round-label-layout.test.mjs` 已删除；R20 = `detect-type-conflicts` / `backfill-bid` 重写为 `.mjs`（旧 `.js` 已 `git rm`）并各自带测试；R21 = `public/_headers` + 中间件的写请求 Origin 闸门 + API 响应统一 `private, no-store`（含 17 例测试）。提交时实测：`npm test` **495/495**、前后端 tsc 0 错、`npm run build` 成功；**`npm run lint` 因 25 条存量错误退出 1**（基线见 R19 完成记录）。

**第三批（R34 + 合包来源顺序 + 下载并发旋钮 + 回填 Action）已提交入库**：R34 = 槽位记号不再当谱面身份、轮次判重改用 `round.id`（见 R34 章节）；`compareTournamentsForSources` **年份降序 → 升序**并抽成可测函数（⚠️ 会改包内合并谱面的 `Version` 字符串 = 玩家成绩身份，见 `CODEX-HANDOFF.md` §5.4）；下载并发从写死的 `4` 改为 `PACK_DOWNLOAD_CONCURRENCY`（默认 8、上限 32）；新增 `.github/workflows/backfill-metadata.yml`（**默认只报告**，`apply=true` 才写数据）；新增 `src/lib/ladderScope.ts`（天梯键型判读与标题高度的唯一实现，含 9 例测试）。提交时实测：`npm test` **519/519**、前后端 tsc 各 0 错、`npm run build` 成功（`/`、`/admin`、`/download` 三条路由静态预渲染）、`npm run lint` 仍 **25 条存量错误**（未新增）、密钥扫描 0 命中。
...
| ✅ R10 | 合包发布完整性 | 全 6 条 + 审查后加固（GC 独立命令/保留期/CLI 拒绝空类型与裸 --offline/**Drive 也版本化**）；未在真实 R2/Actions 上跑过全量 |
| ⚠️ R11 | 谱面身份/指纹与备选副本 | 内容摘要 + 备选等价校验已做；未在真实 R2 上跑过全量 |
| ✅ R12 | workflow/CLI/单类型发布 | 全 6 条已做（第 6 条随版本键一起落地）；未在真实 Actions 上跑过 |
