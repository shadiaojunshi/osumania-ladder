...
export default function FeedbackPage() {
  ...
  // 解绑谱面后要给一句交代：文字表单是按选中项挂 key 的（换目标 = 换草稿键），
  // 一解绑就重挂、输入框变空 —— 而内容其实还在本机另一个草稿键下。不说一声，
  // 玩家会以为内容丢了、在新表单里重写，那段针对某张谱的反馈就永远不会被提交。
  const [unboundFrom, setUnboundFrom] = useState('')
  const select = (row: MapBrowserRow) => { setSelected(row); setUnboundFrom('') }
  ...
  return <div className="min-h-screen bg-gray-50 text-gray-900 dark:bg-neutral-950 dark:text-neutral-100">
    ...
    <main className="mx-auto max-w-6xl space-y-6 px-4 py-8">
      ...
      <div className="flex flex-wrap gap-3"><button className="rounded-lg border px-4 py-2 text-sm aria-pressed:bg-purple-600 aria-pressed:text-white" aria-pressed={mode === 'text'} onClick={() => setMode('text')}>{tr('直接写反馈', 'Write feedback')}</button><button className="rounded-lg border px-4 py-2 text-sm aria-pressed:bg-purple-600 aria-pressed:text-white" aria-pressed={mode === 'map'} onClick={() => setMode('map')}>{tr('选择谱面修改', 'Suggest map changes')}</button>{mode === 'text' && selected && <button className="text-sm underline" onClick={() => { setUnboundFrom(`${selected.tournamentName} · ${selected.roundName} · ${selected.slot}`); setSelected(null) }}>{tr('取消关联谱面', 'Remove map context')}</button>}</div>
      {mode === 'text' && !selected && unboundFrom && <p role="status" className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900 dark:border-amber-900 dark:bg-amber-950 dark:text-amber-200">{tr(`已切换为不关联谱面的反馈。刚才针对「${unboundFrom}」写的内容仍保存在本机草稿里，重新选中该谱面就能继续；下面这张表单是空的。`, `Switched to feedback without a map. What you wrote for "${unboundFrom}" is still stored as a local draft — select that map again to continue. This form below is empty.`)}</p>}
      ...
      <section className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-neutral-800 dark:bg-neutral-950">
        ...
        <div className="divide-y divide-gray-100 dark:divide-neutral-800">{rows.slice(page * PAGE_SIZE, (page + 1) * PAGE_SIZE).map(row => <div key={row.key} className="flex items-center justify-between gap-3 p-4 hover:bg-purple-50/50 dark:hover:bg-purple-950/20"><div className="min-w-0"><p className="text-xs text-gray-500">{row.tournamentName} · {row.roundName}</p><p className="mt-1 break-words text-sm font-medium"><span className="mr-2 text-purple-600 dark:text-purple-300">{row.slot}</span>{row.name}</p><p className="mt-1 text-xs text-gray-500">{row.realType || '—'} · {row.difficulty || '—'}{row.difficultyLn ? ` / LN ${row.difficultyLn}` : ''}{isUsableBeatmapId(row.beatmapId) ? ` · BID ${row.beatmapId}` : ''}</p></div><button className="shrink-0 rounded-lg border border-purple-200 px-3 py-2 text-sm text-purple-700 dark:border-purple-800 dark:text-purple-300" onClick={() => { select(row); setMode('map'); window.scrollTo({ top: 0, behavior: 'smooth' }) }}>{tr('建议修改', 'Suggest')}</button></div>)}</div>
